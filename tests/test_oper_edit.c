...
test_oper_mixed(void **arg)
{
    ...
    for (i = 0; i < 2; i++) {
       ...
       ret = sr_set_item_str(st->sess, "/ex-interfaces:interfaces/interface[name=\'mixed\']/enabled",
        ...
        ret = sr_delete_item(st->sess, "/ex-interfaces:interfaces/interface[name=\'mixed\']/statistics/leaf-test-uint8", 0);
        assert_int_equal(ret, SR_ERR_OK);

        ret = sr_apply_changes(st->sess, 0);
        assert_int_equal(ret, SR_ERR_OK);

        ret = sr_get_data(oper_sess, "/ex-interfaces:*", 0, 0, 0, &data);
        assert_int_equal(ret, SR_ERR_OK);
